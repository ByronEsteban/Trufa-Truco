...
void canta_envido(int tanto_jug, int tanto_ia){
	//if(ia_acepta())
	if(1){
		printf("QUIERO!!!");
		system("cls");
		if (tanto_jug > tanto_ia) printf("Gana el jug\n");
		else printf("Gana la ia\n");
		Sleep(2000);
		return;
	}
	printf("NO QUIERO");
...
void turno_jugador(std::vector<std::string> &opciones, std::vector<Carta> &cartas_tiradas_jug, std::vector<Carta> &cartas_tiradas_ia, std::vector<Carta> &mano_jugador, std::vector<Carta> &mano_ia, int puntos_ronda, int turno){//2.3
  int opcion;
  ...
  while (!mano_jugador.empty()) {
		...
		if(turno == 0){
			...
			else if(opcion == 5) canta_envido(tanto_jug, tanto_ia);
  	...
  	if(opcion < mano_jugador.size()) break; //dejarlo para los casos de tirar carta, NO para truco, flor, envido, retirarse
  }
}
...
void truco(Carta baraja[]){ //2
    ...
    if(jugador_es_mano){
		...
		turno_jugador(opciones, cartas_tiradas_jug, cartas_tiradas_ia, mano_jugador, mano_ia, puntos_ronda, 0);
		//ia_canta_envido
		//ia_canta_truco
		...
		if(cartas_tiradas_jug[0].posicion > cartas_tiradas_ia[0].posicion){
			...
			tira_carta_ia(cartas_tiradas_jug[0], cartas_tiradas_ia, mano_ia, 1);
	      	Sleep(1500);
			turno_jugador(opciones, cartas_tiradas_jug, cartas_tiradas_ia, mano_jugador, mano_ia, puntos_ronda, 1);
		} else {
		...
		if(cartas_tiradas_jug[1].posicion > cartas_tiradas_ia[1].posicion){
			...
			tira_carta_ia(cartas_tiradas_jug[1], cartas_tiradas_ia, mano_ia, 2);
	      	Sleep(1500);
			turno_jugador(opciones, cartas_tiradas_jug, cartas_tiradas_ia, mano_jugador, mano_ia, puntos_ronda, 2);
		} else {
