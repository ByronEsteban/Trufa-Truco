...
void tira_carta(int opcion, std::vector<Carta> &cartas_tiradas_jug, Carta carta_tirada, std::vector<Carta> &mano_jugador, std::vector<std::string> &opciones, int turno){ //2.3.1
  cartas_tiradas_jug.push_back(carta_tirada);
	...
	mano_jugador.erase(mano_jugador.begin() + opcion - 1);
...
void turno_jugador(std::vector<Carta> &cartas_tiradas_jug, std::vector<Carta> &cartas_tiradas_ia, std::vector<Carta> &mano_jugador, int puntos_ronda_jug, int puntos_ronda_ia, int turno){//2.3
  int opcion;
  while (!mano_jugador.empty()) {
    ...
    switch (opcion) {
      case 1:
      	printf("\nHOLA\n");
        tira_carta(opcion, cartas_tiradas_jug, mano_jugador[opcion], mano_jugador, opciones, turno);
        Sleep(1500);
